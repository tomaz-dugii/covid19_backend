...
export async function deposit(req: Request, res: Response) {
    ...
    if (name == undefined)
    {
        ...
        return res.send('Missing player\'s name /deposit?name=playername&session=sessionID&amount=value');
    }
    else if (sessionID == undefined)
    {
        console.log('Missing sessionID for player: ' + name + ' \n /deposit?name=playername&session=sessionID&amount=value');
        return res.send('Missing sessionID /deposit?name=playername&session=sessionID&amount=value');
    }
    else if (amountDef == undefined)
    {
        console.log('Missing money transaction amount for player: ' + name + ' \n /deposit?name=playername&session=sessionID&amount=value');
        return res.send('Missing money transaction amount /deposit?name=playername&session=sessionID&amount=value');
    }
    ...
    db.tx(t => {
            ...
            .then(function (user) {
                ...
                console.log("Preparing deposit for playerID: " + playerID + ' ,with current balance of: ' + balance + ' ,newBalance: ' + newBalance);
                return t.none('UPDATE wallet SET "balance" = $1 WHERE "playerID" = $2 ', [newBalance, playerID])
                .then(function () {
                    return t.none('INSERT INTO "transactionHistory" ("playerID", "sessionID", "balanceBefore", "deposit") values ($1,$2,$3,$4) ', [playerID, sessionID, balance, amount]);
                });
            });
            // returning a promise that determines a successful transaction:
        })
        .then(data => {
            console.log("Deposited amount to wallet for player Name: " + name + " ,amount: " + amount + ' ,SessionID: ' + sessionID + ' ,previousBalance: ' + balance);
            return res.send('Deposited amount to wallet for player: ' + name + " ,amount: " + amount + ' ,SessionID: ' + sessionID + ' ,previousBalance: ' + balance);
        })
        .catch(error => {
            // failure, ROLLBACK was executed
            console.log('ERROR:', error)
            return res.send('Error depositing money to player wallet');
    });
